...
// TERRARIUM with extention png in Web Mercator projection, TERRARIUMxTERRARIUM
// tiles
...
void mapZenLoader::clearMaps() {
  maps.clear();
  // mapImages.clear();
}

void mapZenLoader::createMapImage(float lat_, float lon_, int zoom_, int width_,
                                  int heigth_, string type_) {
  mapZenBlock temp;
  if (type_ == "Terrarium") {
    temp.mapImage.allocate((width_ / TERRARIUM) * TERRARIUM,
                           (heigth_ / TERRARIUM) * TERRARIUM, OF_IMAGE_COLOR);
  } else if (type_ == "Normal") {
    temp.mapImage.allocate((width_ / TERRARIUM) * TERRARIUM,
                           (heigth_ / TERRARIUM) * TERRARIUM,
...
void mapZenLoader::loadTiles(int index_, string type_) {
  ...
  for (int x = 0; x < w / TERRARIUM; x++) {
    for (int y = 0; y < h / TERRARIUM; y++) {
      loadFromURL(
          ofVec2f(x, y), maps.at(index_).tiles[(y * (w / TERRARIUM)) + x],
          loadUrl + ofToString(maps.at(index_).zoom) + "/" +
              ofToString(maps.at(index_).pos.x + x) + "/" +
              ofToString(maps.at(index_).pos.y + y) + ".png?api_key=" + MZKEY,
          index_);
      // insert your key ;)
    }
  }
...
void mapZenLoader::addTile(ofImageLoaderEntry &entry_) {
  ...
  unsigned char *pixelData =
      maps.at(entry_.index).mapImage.getPixels().getData();
  int imgR, imgG, imgB, imgA, imgArr;
  int pixSize = TERRARIUM * TERRARIUM * channels;

  imgArr =
      (pixSize * (int(maps.at(entry_.index).mapImage.getWidth() / TERRARIUM) *
                  entry_.pos.y)) +
      ((TERRARIUM * channels) * entry_.pos.x);
  ...
  for (int i = 0; i < TERRARIUM; i++) {
    for (int j = 0; j < TERRARIUM; j++) {
      ...
      int offsetCol = (imgR / (TERRARIUM * channels)) *
                      (maps.at(entry_.index).mapImage.getWidth() * channels);
...
ofVec2f mapZenLoader::to3857(ofVec2f in_, float zoom_) {
  ...
  temp.y =
      floor((1 - log(tan(in_.x * PI / 180) + 1 / cos(in_.x * PI / 180)) / PI) /
            2 * pow(2, zoom_));

  return temp;
}

vector<ofImage *> *mapZenLoader::getMapImagesPtr() { return &mapImages; }
...
//--------------------------------------------------------------
void mapZenLoader::loadFromURL(ofVec2f pos, ofImage &image, string url,
                               int index_) {
  nextID++;
...
void mapZenLoader::threadedFunction() {
  ...
  while (images_to_load_from_disk.receive(entry)) {
    if (entry.image->load(entry.filename)) {
      ...
      ofLogError("mapZenLoader")
          << "couldn't load file: \"" << entry.filename << "\"";
    }
  }
...
void mapZenLoader::urlResponse(ofHttpResponse &response) {
  ...
  if (response.status == 200) {
    ...
    // log error.
    ofLogError("mapZenLoader")
        << "couldn't load url, response status: " << response.status;
