...
void PolyMap::loadMap(ofImage &terrain_, int steps_) {
  ...
  for (int i = 1; i < steps_; i++) {
    ...
    for (int k = 0; k < contourFinder.blobs.size(); k++) {
      ...
      ofVec3f trans = ofVec3f(0 - 256 * 3, -256 * 3, 0);
      ...
      tempPath.moveTo(tempLineFirst.getVertices().front() + trans);
      tempPath.curveTo(tempLineFirst.getVertices().front() + trans);

      for (int l = 0; l < tempLineFirst.getVertices().size(); l++) {
        tempPath.curveTo(tempLineFirst.getVertices().at(l) + trans);
      }
      tempPath.lineTo(tempLineFirst.getVertices().back() + trans);

      tempPath.setFillColor(ofColor(layerSteps * i));
      tempPath.setStrokeColor(ofColor(0, 0, 0, 100));
      tempPath.setStrokeWidth(2.50);
